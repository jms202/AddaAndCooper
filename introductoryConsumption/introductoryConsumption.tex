...
The code in \texttt{markovCakeSpline.py} solves the same problem but using spline interpolation to approximate the value function and expected marginal utility. This code demonstrates some of the challenges in using spline approximation: monotonicity and convexity are not maintained so there are problems with the solution, particularly when using the Euler equation approach to solve for the value function. Of course, there may may be an error in the code but I haven't been able to track one down!
