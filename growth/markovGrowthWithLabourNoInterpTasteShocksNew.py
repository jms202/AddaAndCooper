...
class Model:
    ...
    def Angp(self, value):
        ...
        if all([val for val in self.isSet4.values()]):
            self.currReturn = np.empty((self.kngp, self.Angp, self.Sngp, self.kngp))
            self.ixnBest = np.empty((self.kngp, self.Angp, self.Sngp, self.kngp))
            self.EV = np.zeros((self.kngp, self.Angp, self.Sngp))
            self.ixk1policy = np.empty((self.kngp, self.Angp, self.Sngp), dtype=np.int32)
            self.ixnpolicy = np.empty((self.kngp, self.Angp, self.Sngp), dtype=np.int32)
            ...
            for ixA, A in enumerate(self.A):
                for ixS, S in enumerate(self.S):
                    for ixk1, k1 in enumerate(self.kgrid):
                        ...
                        if len(c) == 0:
                            self.ixnBest[ixk, ixA, ixS, ixk1] = -1
                            continue
                        ...
                        ixnOpt = np.argmax(u)
                        self.ixnBest[ixk, ixA, ixS, ixk1] = ixnOpt + self.nngp - len(c)

                        # Store maximum
                        self.currReturn[ixk, ixA, ixS, ixk1] = u[ixnOpt]
    ...
    def guessSln(self):
        ...
        # We already calculated the current return from all combinations of k, A, S and k1
        # Dimensions: EV(k, A_1, S_1), currReturn[K, A, S, k1)
        self.EV[:,:,:] = self.p @ self.currReturn[:,:,:,0] @ self.q.T


    # Find next iteration
    def nextIter(self, EVNext):
    
        VNext = np.empty((self.kngp, self.Angp, self.Sngp))
        #temp = np.empty((self.kngp, self.Angp, self.Sngp))
        ...
        for ixk in range(self.kngp):
            for ixA in range(self.Angp):
                for ixS in range(self.Sngp):
                    ...
                    objective = self.currReturn[ixk, ixA, ixS, :] + (self.beta*self.EV[:, ixA, ixS])
                    ixk1Opt = np.argmax(objective)
                    VNext[ixk, ixA, ixS] = objective[ixk1Opt]


        # Now integrate over uncertain shocks
        EVNext[:,:,:] = self.p @ VNext @ self.q.T
    ...
    def solve(self, iter=1000, verbose=False):
        ...
        EVNext = np.empty((self.kngp, self.Angp, self.Sngp))
        ...
        for ixk in range(self.kngp):
            for ixA in range(self.Angp):
                for ixS in range(self.Sngp):
                    ...
                    self.ixk1policy[ixk, ixA, ixS] = ixk1Opt
                    self.ixnpolicy[ixk, ixA, ixS] = self.ixnBest[ixk, ixA, ixS, ixk1Opt]
    ...
    def plotValue(self, ixS=0):
        ...
        fig, ax = plt.subplots()
        ax.plot(self.kgrid, self.EV[:,:,ixS])
    ...
    def plotk1Policy(self, ixS=0):
        ...
        fig, ax = plt.subplots()
        ax.plot(self.kgrid, self.kgrid[self.ixk1policy[:,:,ixS]])
        ax.set_xlabel('k', fontsize=14)
    ...
    def plotnPolicy(self, ixS=0):
        ...
        fig, ax = plt.subplots()
        ax.plot(self.kgrid, self.ngrid[self.ixnpolicy[:,:,ixS]])
        ax.set_xlabel('k', fontsize=14)
    ...
    def simulate(self, sims, n=1000, horiz=100):
        ...
        for i in range(n):
            ...
            sims['ixk1'][i, 0] = self.ixk1policy[sims['ixk'][i, 0], sims['ixA'][i, 0], sims['ixS'][i, 0]]
            ...
            for h in range(1, horiz):
                ...
                # Calculate optimal n
                sims['ixn'][i, h] = self.ixnpolicy[sims['ixk'][i, h], sims['ixA'][i, h], sims['ixS'][i, h]]
                # Calculate optimal k'
                sims['ixk1'][i, h] = self.ixk1policy[sims['ixk'][i, h], sims['ixA'][i, h], sims['ixS'][i, h]]
