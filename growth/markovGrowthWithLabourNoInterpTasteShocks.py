...
class Model:
    ...
    def __init__(self, alpha=0.75, beta=0.9, gamma=1.5, delta=0.3, rho=0.9, sigma_u=0.1, phi=0.5, sigma_v = 0.05, B=2, kRange=0.8, kngp=101, nngp=96, Angp=7, Sngp=3):
        ...
        self.isSet4 = {'Angp':False, 'kngp':False, 'Sngp':False}
        self.isSet5 = {'phi':False, 'sigma_v':False, 'Sngp':False}
        self.nMin = 0.05
        self.nMax = 1.0
        self.nngp = nngp
    ...
    def Angp(self, value):
        ...
        if all([val for val in self.isSet4.values()]):
            self.currReturn = np.empty((self.Angp, self.Sngp, self.kngp, self.kngp))
            self.ixnBest = np.empty((self.Angp, self.Sngp, self.kngp, self.kngp))
            self.EV = np.zeros((self.Angp, self.Sngp, self.kngp))
            self.ixk1policy = np.empty((self.Angp, self.Sngp, self.kngp), dtype=np.int32)
            self.ixnpolicy = np.empty((self.Angp, self.Sngp, self.kngp), dtype=np.int32)
    ...
    def phi(self, value):
        self._phi = value
        self.isSet5['phi'] = True
        if all([val for val in self.isSet5.values()]):
            self.S, self.q = approx_markov(rho=self.phi, sigma_u=self.sigma_v, m=3, n=self.Sngp)
    ...
    # Utility
    def util(self, c, n, S):
        return S * (np.log(c) - self.B * (n**(1.0 + 1.0/self.gamma)) / (1.0 + 1.0/self.gamma))
            ...
            for ixS, S in enumerate(self.S):
                for ixk, k in enumerate(self.kgrid):
                    for ixk1, k1 in enumerate(self.kgrid):
                        ...
                        if len(c) == 0:
                            self.ixnBest[ixA, ixS, ixk, ixk1] = -1
                            continue
                        
                        # calculate current-period utility at each feasible labour supply point
                        u = self.util(c, self.ngrid[-len(c):], S)

                        # Store optimal labour supply index
                        ixnOpt = np.argmax(u)
                        self.ixnBest[ixA, ixS, ixk, ixk1] = ixnOpt + self.nngp - len(c)

                        # Store maximum
                        self.currReturn[ixA, ixS, ixk, ixk1] = u[ixnOpt]
    ...
    def guessSln(self):
        ...
        # We already calculated the current return from all combinations of A, S, k and k1
        # Just need to integrate over uncertain shocks
        for ixA in range(self.Angp):
            self.EV[ixA,:,:] = self.q @ self.currReturn[ixA,:,:,0]
        for ixS_1 in range(self.Sngp):
            self.EV[:,ixS_1,:] = self.p @ self.EV[:,ixS_1,:]


    # Find next iteration
    def nextIter(self, EVNext):
    
        VNext = np.empty((self.Angp, self.Sngp, self.kngp))
        temp = np.empty((self.Angp, self.Sngp, self.kngp))
    
        # Calculate value function assuming shock is known
        for ixA in range(self.Angp):
            for ixS in range(self.Sngp):
                for ixk in range(self.kngp):

                    # For each value of (A, k), work out optimal k1
                    objective = self.currReturn[ixA, ixS, ixk, :] + (self.beta*self.EV[ixA, ixS, :])
                    ixk1Opt = np.argmax(objective)
                    VNext[ixA,ixS, ixk] = objective[ixk1Opt]


        # Now integrate over uncertain shocks
        for ixA in range(self.Angp):
            temp[ixA,:,:] = self.q @ VNext[ixA,:,:]
        for ixS_1 in range(self.Sngp):
            temp[:,ixS_1,:] = self.p @ temp[:,ixS_1,:]
        np.copyto(EVNext, temp)
    ...
    def solve(self, iter=1000, verbose=False):
        ...
        EVNext = np.empty((self.Angp, self.Sngp, self.kngp))
        ...
        for ixA in range(self.Angp):
            for ixS in range(self.Sngp):
                for ixk in range(self.kngp):
                    ...
                    self.ixk1policy[ixA, ixS, ixk] = ixk1Opt
                    self.ixnpolicy[ixA, ixS, ixk] = self.ixnBest[ixA, ixS, ixk, ixk1Opt]



    # Plot value function
    def plotValue(self, ixS=0):
    
        fig, ax = plt.subplots()
        ax.plot(self.kgrid, self.EV[:,ixS,:].T)
    ...
    def plotk1Policy(self, ixS=0):
    
        fig, ax = plt.subplots()
        ax.plot(self.kgrid, self.kgrid[self.ixk1policy[:,ixS,:].T])
        ax.set_xlabel('k', fontsize=14)
    ...
    def plotnPolicy(self, ixS=0):
    
        fig, ax = plt.subplots()
        ax.plot(self.kgrid, self.ngrid[self.ixnpolicy[:,ixS,:].T])
        ax.set_xlabel('k', fontsize=14)
    ...
    def drawShocks(self, n=1000, horiz=100):
        ...
        sims = {'ixA':np.empty((n, horiz), dtype=np.int32), 'ixS':np.empty((n, horiz), dtype=np.int32)}
        sims['ixA'][:, 0] = np.random.choice(self.Angp, size=n)
        sims['ixS'][:, 0] = np.random.choice(self.Sngp, size=n)

        for i in range(n):
            for h in range(1, horiz):

                sims['ixA'][i, h] = np.random.choice(self.Angp, p = self.p[sims['ixA'][i, h-1], :])
                sims['ixS'][i, h] = np.random.choice(self.Sngp, p = self.q[sims['ixS'][i, h-1], :])
    ...
    def simulate(self, sims, n=1000, horiz=100):
        ...
        for i in range(n):
            ...
            sims['ixk1'][i, 0] = self.ixk1policy[sims['ixA'][i, 0], sims['ixS'][i, 0], sims['ixk'][i, 0]]
            ...
            for h in range(1, horiz):
                ...
                # Calculate optimal n
                sims['ixn'][i, h] = self.ixnpolicy[sims['ixA'][i, h], sims['ixS'][i, h], sims['ixk'][i, h]]
                # Calculate optimal k'
                sims['ixk1'][i, h] = self.ixk1policy[sims['ixA'][i, h], sims['ixS'][i, h], sims['ixk'][i, h]]
        

        # Now calculate other variables
        sims['A'] = self.A[sims['ixA']]
        sims['S'] = self.S[sims['ixS']]
