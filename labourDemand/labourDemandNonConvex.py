...
class Model:
    ...
    def __init__(self, alpha=0.7, beta=0.95, w=0.05, w0=1.5, w1=0.19, w2=0.03, F=0.1, rho=0.8, sigma=0.1, engp=7, Angp=7, hngp=7):
    ...
    def allocateSln(self):
        if all([val for val in self.slnReady.values()]):
            ...
            self.zpolicy = np.empty((self.Angp, self.engp), dtype=np.bool_)
    ...
    def periodReturn(self, A, Le, e, h, adjust):
        return self.revenue(A, e, h) - self.compensation(e, h) - (self.F * adjust)
    ...
    def guessSln(self):
        ...
        for ixA, A in enumerate(self.A):
            for ixLe, Le in enumerate(self.egrid):
                profitIfAdjust = np.max(self.periodReturn(A, Le, emesh, hmesh, adjust=True))
                profitNotAdjust = np.max(self.periodReturn(A, Le, Le, self.hgrid, adjust=False))
                V[ixA,ixLe] = max(profitIfAdjust, profitNotAdjust)
    ...
    def nextIter(self, EVNext):
        ...
        for ixA, A in enumerate(self.A):
            for ixLe, Le in enumerate(self.egrid):
                ...
                VIfAdjust = np.max(self.periodReturn(A, Le, emesh, hmesh, adjust=True) + (self.beta * EVmesh))
                VNotAdjust = np.max(self.periodReturn(A, Le, Le, self.hgrid, adjust=False) + (self.beta * self.EV[ixA, ixLe]))
                VNext[ixA,ixLe] = max(VIfAdjust, VNotAdjust)
    ...
    def findPolicy(self):
        ...
        for ixA, A in enumerate(self.A):
            for ixLe, Le in enumerate(self.egrid):
                ...
                # Find value if adjust
                objectiveIfAdjust = self.periodReturn(A, Le, emesh, hmesh, adjust=True) + (self.beta*EVmesh)
                maxpos = np.unravel_index(np.argmax(objectiveIfAdjust), emesh.shape)
                ixhOptIfAdjust = maxpos[0]
                ixeOptIfAdjust = maxpos[1]
                VIfAdjust = objectiveIfAdjust[ixhOptIfAdjust, ixeOptIfAdjust]

                # Find value if not adjust
                objectiveNotAdjust = self.periodReturn(A, Le, Le, self.hgrid, adjust=False) + (self.beta * self.EV[ixA, ixLe])
                ixhOptNotAdjust = np.argmax(objectiveNotAdjust)
                ixeOptNotAdjust = ixLe
                VNotAdjust = objectiveNotAdjust[ixhOptNotAdjust]

                # Find optimum
                if (VIfAdjust > VNotAdjust):
                    self.zpolicy[ixA, ixLe] = True
                    self.ixhpolicy[ixA, ixLe] = ixhOptIfAdjust
                    self.ixepolicy[ixA, ixLe] = ixeOptIfAdjust
                else:
                    self.zpolicy[ixA, ixLe] = False
                    self.ixhpolicy[ixA, ixLe] = ixhOptNotAdjust
                    self.ixepolicy[ixA, ixLe] = ixeOptNotAdjust
